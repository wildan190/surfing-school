<x-layouts.app :title="__('Edit Paket Edukasi')">
    <div class="p-6 bg-white dark:bg-gray-900 shadow-lg rounded-lg">
        ...
        <form action="{{ route('paket-edukasi.update', $paketEdukasi->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label class="block text-gray-700 dark:text-gray-300">Nama Paket</label>
                <input type="text" name="nama_paket" value="{{ $paketEdukasi->nama_paket }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800 dark:text-gray-200" required>
            </div>
